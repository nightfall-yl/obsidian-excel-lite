...
> Svelte 4+ 使用 `mount()` / `unmount()` API 替代了旧版 `new Component()` 构造方式。以下示例兼容 Svelte 4 和 Svelte 5。
...
> ⚠️ **注意**：使用框架渲染设置面板时，无法使用 `SettingGroup` 组件。如果设置项需要分组卡片样式，建议改用原生 `SettingGroup`（参考 [ui-patterns.md](ui-patterns.md) §4）。仅在需要高度自定义交互 UI 时才使用 Vue 渲染设置面板。
