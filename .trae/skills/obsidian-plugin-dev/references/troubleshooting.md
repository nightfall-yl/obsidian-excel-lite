...
### SettingGroup 不显示卡片背景

**症状**：使用 `SettingGroup` 后，设置项没有分组卡片效果。

**原因**：`SettingGroup` 的卡片背景由 Obsidian 内置 CSS 自动处理。如果你在 `styles.css` 中覆盖了 `.setting-group` 相关样式，可能导致冲突。

**解决**：删除 `styles.css` 中所有 `.setting-group`、`.setting-item` 的自定义样式，让 Obsidian 原生样式生效。

### 移动端亮色模式下卡片融入页面背景

**症状**：设置面板卡片在移动端亮色模式下几乎看不见，与页面背景融为一体。

**原因**：移动端亮色模式下，`--background-primary-alt` 与 `--background-primary` 差异很小。

**解决**：使用 `--background-primary` 作为移动端亮色模式的卡片背景，通过 CSS 媒体查询区分：

```css
@media (max-width: 720px) {
  .theme-light .your-card {
...
**症状**：自定义样式被 Obsidian 主题覆盖，或覆盖了 Obsidian 原生样式。

**原因**：Obsidian 主题的 CSS 特异性可能高于插件样式。

**解决**：
1. 使用更具体的选择器（如 `.workspace-leaf-content[data-mode="source"] .your-class`）
2. 避免使用 `!important`（除非绝对必要）
3. 始终使用 Obsidian CSS 变量而非硬编码颜色值
