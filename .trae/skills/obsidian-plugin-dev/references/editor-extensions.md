...
> **`@ts-expect-error` 规则**：Obsidian 未公开 `editor.cm` 的类型定义，访问时必须使用 `@ts-expect-error` 标注。但在 CM6 扩展代码内部（State Field、View Plugin 等），`EditorView` 是通过 `@codemirror/view` 导入的，类型已知，**不需要** `@ts-expect-error`。

```typescript
// 从 editorCallback 中访问（需要 @ts-expect-error）
this.addCommand({
  id: 'my-command',
  name: 'My Command',
...
// 在 CM6 扩展内部访问（不需要 @ts-expect-error，EditorView 由参数传入）
class MyPlugin implements PluginValue {
  constructor(view: EditorView) {
    // view 已经是 EditorView 类型，无需标注
  }
}
```
...
// line.number, line.from, line.to, line.text
