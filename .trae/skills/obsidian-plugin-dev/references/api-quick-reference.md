...
class PluginSettingTab extends SettingTab {
  ...
  display(): void;   // 构建设置 UI，面板打开时调用
  hide(): void;      // 清理资源，面板关闭时调用（如移除自定义 class、框架 unmount）
}
```

### SettingGroup（设置分组）⚠️ 推荐使用

> **设置面板必须使用 `SettingGroup`**，而非直接用 `Setting` 平铺。详见 [ui-patterns.md](ui-patterns.md) §4 和 §10.6。

```typescript
class SettingGroup {
  constructor(containerEl: HTMLElement);
  setHeading(heading: string | DocumentFragment): this;
  addSetting(cb: (setting: Setting) => any): this;
  addSearch(cb: (component: SearchComponent) => any): this;
